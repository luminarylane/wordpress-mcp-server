...
describe("senseResult", () => {
  it("wraps data with EXTCONTENT markers", () => {
    const result = senseResult({ text: "hello" }, "WordPress");
    expect(result.content).toHaveLength(1);
    expect(result.content[0].type).toBe("text");
  ...
  it("includes source label in header", () => {
    const result = senseResult({}, "WordPress");
    expect(result.content[0].text).toContain(
      "Untrusted content from WordPress",
    );
  });
  ...
  it("safely contains adversarial content with fake markers", () => {
    ...
    const result = senseResult(adversarial, "WordPress");
